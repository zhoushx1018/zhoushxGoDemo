...
func main() {
	...
	// 主 goroutine，  读取 reads 和 writes 两个chan 的数据，并处理
	//  处理之后，回写 内部私有chan(read.resp和 write.resp)
	go func() {
	...
	//100个 read goroutine， 往 reads chan 写入数据，随后等待 read.resp 事件
	for r := 0; r < 100; r++ {
		go func() {
			for {
				read := &readOp{
	...
	//10个 write goroutine， 往 writes chan 写入数据，随后等待 write.resp 事件
	for w := 0; w < 10; w++ {
		go func() {
			for {
				write := &writeOp{
	...
	//打印 read goroutine 和 write goroutine 的处理次数
